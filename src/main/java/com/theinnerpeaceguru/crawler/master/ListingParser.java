...
//    final Elements properties = document.select("list medium-block-grid-3 > li[class~=center card]");
    final Elements properties = document.select(".entity-box");
  ...
  private static Property parseProperty(Element e) {
    ...
    property.setTitle(textOrNull(e.select(".entity-title").first()));
    property.setLocation(textOrNull(e.select(".show-for-small").first()));
